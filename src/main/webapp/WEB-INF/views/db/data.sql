...
(
    ...
    CONSTRAINT REPLY_PK PRIMARY KEY (R_NO)
);
/
...
COMMENT ON COLUMN REPLY.C_NO IS '클럽번호'
/
...
COMMENT ON COLUMN REPLY.R_DELETE IS '삭제여부'
/

COMMENT ON COLUMN REPLY.R_POSTDATE IS '작성시간'
/

ALTER TABLE REPLY(BOARD)
    ADD CONSTRAINT FK_REPLY_C_NO_CLUB_C_NO FOREIGN KEY (C_NO)
        REFERENCES CLUB (C_NO)
/

ALTER TABLE REPLY(BOARD)
    ADD CONSTRAINT FK_REPLY_M_NO_MEMBER_M_ FOREIGN KEY (M_NO)
        REFERENCES MEMBER (M_NO)
/
