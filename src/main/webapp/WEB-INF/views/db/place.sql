...
SELECT DISTINCT A.P_NO, A.RVCOUNT,  PC.PC_NAME, MIN(PO.PO_DAYPRICE) AS MINPRICE, MAX(PO.PO_MAX) AS MAXPEOPLE,  P2.P_TITLE,  P2.P_IMG,  P2.P_ADDR
		  FROM (SELECT P.P_NO, P.PC_NO, COUNT(RV.P_NO) AS RVCOUNT
				  FROM PLACE P LEFT JOIN REVIEW RV ON P.P_NO = RV.P_NO
				 WHERE P.P_CONFIRM = 1 
				   AND P.P_DELETE = 0
			  ...
			  GROUP BY P.P_NO, P.PC_NO) A LEFT JOIN PLACECATEGORY PC ON A.PC_NO = PC.PC_NO
										  LEFT JOIN PLACEOPTION PO ON A.P_NO = PO.P_NO
										  LEFT JOIN PLACE P2 ON A.P_NO = P2.P_NO
	  GROUP BY A.P_NO, A.RVCOUNT, PC.PC_NAME
	  ORDER BY A.P_NO
	  
SELECT P.*, PC.PC_NAME, MIN(PO.PO_DAYPRICE) AS MINPRICE, MAX(PO.PO_MAX) AS MAXPEOPLE
